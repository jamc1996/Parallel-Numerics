int comm_steps = int_log2(nprocs);
	...
	decomp1d(0, n, nprocs, myid, &s, &e);
	DenseMatrix W = CreateNullMatrix(1+e-s,b);
	for (j=0; j<b; j++)
	{
		memcpy(W.entry[j],&A.entry[j][s],sizeof(double)*(1+e-s));
	}
...
//	printf("(myid is %d) and level is %d\n",myid,level);
...
void caqr(DenseMatrix* A, DenseMatrix* Rfin, DenseMatrix *Qfin, int nprocs, int myid, int b)
{
	int s,e;
	int i,j,k,p;
	int os, oe;
...
for(j=0; j<A->nColumns/b; j++){
	//Each processor takes it's chunk of matrix.
	decomp1d(j*b, A->nRows, nprocs, myid, &s, &e);
	DenseMatrix W = CreateNullMatrix(1+e-s,b);
	for (i=0; i<b; i++)
	{
		memcpy(W.entry[i],&A->entry[(j*b)+i][s],sizeof(double)*(1+e-s));
	}
	...
	for (p=j;p<A->nColumns/b;p++)
	{
		if (myid == 1){
		printf("j = %d and p = %d\n",j,p);}
		for (i=0; i<b; i++)
		{
			memcpy(W.entry[i],&A->entry[(p*b)+i][s],sizeof(double)*(1+e-s));
		}
		caQtA(Q, &W, myid, comm_steps, mylevel, b, 0,s);
		for (i = 0; i<b; i++)
		{
